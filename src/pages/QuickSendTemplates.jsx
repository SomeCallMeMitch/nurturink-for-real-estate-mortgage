...
export default function QuickSendTemplates() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch Quick Send Templates based on visibility rules
      ...
      const allQuickSendTemplates = [...personal, ...organization, ...platform];
      setQuickSendTemplates(allQuickSendTemplates);
      
      // Fetch related data for display
      const templateIds = [...new Set(allQuickSendTemplates.map(t => t.templateId).filter(Boolean))];
      const cardDesignIds = [...new Set(allQuickSendTemplates.map(t => t.cardDesignId).filter(Boolean))];
      
      if (templateIds.length > 0) {
        const templateList = await base44.entities.Template.filter({ id: { $in: templateIds } });
        setTemplates(templateList);
      }
      
      if (cardDesignIds.length > 0) {
        const designList = await base44.entities.CardDesign.filter({ id: { $in: cardDesignIds } });
  ...
  const filteredTemplates = useMemo(() => {
    ...
    } else if (filters.viewMode === 'platform') {
      filtered = filtered.filter(t => t.type === 'platform');
    }
    
    // Purpose filter
    if (filters.purpose !== 'all') {
      filtered = filtered.filter(t => t.purpose === filters.purpose);
    }
    ...
    if (filters.search.trim()) {
      ...
      filtered = filtered.filter(t => {
        ...
        const linkedTemplate = templateMap[t.templateId];
  ...
  const getPreviewSnippet = (qst) => {
    ...
    const linkedTemplate = templateMap[qst.templateId];
    if (linkedTemplate?.content) {
      return linkedTemplate.content.substring(0, 100) + (linkedTemplate.content.length > 100 ? '...' : '');
    }
