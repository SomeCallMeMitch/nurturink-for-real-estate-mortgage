...
export default function MobileClients() {
  ...
  const handleClientTap = (client) => {
    navigate(`/MobileClients/${client.id}`);
  };

  const handleAddClient = () => {
    navigate('/MobileClients/add');
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 pb-24">
        ...
        {filteredClients.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-500 font-medium">
              {searchQuery ? 'No clients match your search' : activeFilter === 'favorites' ? 'No favorites yet' : 'No clients yet'}
            </p>
          </div>
        ) : (
      ...
      <button
        ...
        className="fixed bottom-24 right-6 w-14 h-14 bg-[#c87533] text-white rounded-full shadow-lg flex items-center justify-center hover:bg-[#b5682e] active:bg-[#a55a28] transition-colors z-20"
      >
