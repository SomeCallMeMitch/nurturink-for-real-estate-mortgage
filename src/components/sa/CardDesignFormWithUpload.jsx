 ...
 * Multi-select dropdown with checkboxes for card design categories.
 */
function CategoryDropdown({ categories, selectedIds, onToggle }) {
  ...
  return (
    <div className="w-[220px] shrink-0" ref={dropdownRef}>
      ...
      <button
        ...
        className="mt-1 w-full flex items-center justify-between gap-2 h-9 px-3 border border-gray-300 rounded-md bg-white text-sm hover:border-gray-400 transition-colors"
      >
      ...
      {/* Dropdown panel */}
      {isOpen && (
        <div className="absolute z-50 mt-1 w-[220px] bg-white border border-gray-200 rounded-lg shadow-lg py-1 max-h-[200px] overflow-y-auto">
          {categories.length === 0 ? (
      ...
      {selectedCount > 0 && (
        <div className="flex flex-wrap gap-1 mt-1">
          {selectedNames.slice(0, 3).map((name) => (
            <span key={name} className="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded">
              {name}
            </span>
          ))}
          {selectedCount > 3 && (
            <span className="text-[10px] text-gray-500">+{selectedCount - 3} more</span>
          )}
        </div>
      )}
    </div>
 ...
 * - Row 2: Inside | Outside | [Front + Back] with Print-Ready files below
 * - Row 3: Scribe ZIP (edit mode only)
 */
