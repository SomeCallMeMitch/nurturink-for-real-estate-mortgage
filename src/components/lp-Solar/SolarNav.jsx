...
/**
 * SolarNav — sticky nav bar
 * Spec: SECTION 1: NAV
 * bg #2a4a7a, 64px height, logo Sora 1.55rem,
 * links "Why NurturInk" → #why-nurturink, "ROI Calculator" → #calculator
 * CTA "Get Free Sample" → #get-sample
 * Mobile: hide links, keep logo + CTA, padding 0 20px
 */
...
function NavLink({ href, children }) {
  return (
    <a
      href={href}
      className="font-lato solar-nav-link"
      style={{
    ...
    >
      ...
      <style>{`
        @media (max-width: 768px) {
          .solar-nav-link { display: none !important; }
        }
      `}</style>
    </a>
  );
