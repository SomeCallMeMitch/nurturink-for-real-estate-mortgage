...
export default function PlaceholderModal({ onPlaceholderSelect }) {
  ...
  if (!isOpen) {
    return (
      <Button
        ...
        variant="outline"
        className="gap-2 px-6 py-2.5 text-[15px] font-medium border border-border rounded-md hover:bg-muted"
      >
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed z-[1000] w-[610px] max-h-[550px] bg-card border-2 border-border rounded-lg shadow-2xl flex flex-col"
        style={{
          left: 'calc(50% - 305px)',
          top: '120px'
        }}
      >
        {/* Fixed Header */}
        <div className="flex-shrink-0 px-5 py-[15px] border-b border-border">
          {/* Close Button */}
          <button
            onClick={handleClose}
            className="absolute top-[15px] right-[15px] text-muted-foreground hover:text-foreground text-2xl px-2.5 py-1.5 leading-none"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <div className="relative mb-2.5">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 text-[15px] border border-border rounded-md focus:outline-none focus:border-primary bg-background text-foreground"
            />
          </div>

          {/* Context Note */}
          <p className="text-[14px] text-foreground mt-2.5">
            These values come from the <strong>client record</strong> you're sending the note to.
          </p>
        </div>

        {/* Fixed Tabs */}
        <div className="flex-shrink-0 bg-muted px-5 py-3 border-b border-border flex gap-2">
          {[
            ...
            <div className="space-y-[15px]">
              {filteredSections.map((section, sectionIndex) => (
                <div key={sectionIndex}>
                  {/* Section Header */}
                  <h3 className="text-[13px] font-semibold text-foreground uppercase tracking-wide mb-1.5 pb-1 border-b border-border">
                    {section.title}
                  </h3>
                  ...
                  <div className="space-y-1">
                    {section.placeholders.map((placeholder, pIndex) => (
                      <button
                        ...
                        className="w-full grid grid-cols-[200px_1fr] gap-5 px-3.5 py-2 rounded hover:bg-muted transition-transform hover:translate-x-0.5"
                      >
                        {/* Code */}
                        <code className="text-left font-['Courier_New'] text-[15px] font-bold text-foreground bg-muted border border-border px-2.5 py-1.5 rounded">
                          {placeholder.code}
                        </code>
