...
import RoofingDiffCard from './RoofingDiffCard';

/**
 * RoofingDifferentiators — SECTION 4: WHY NURTURINK
 * Cloned from SolarDifferentiators
 */
const cards = [
...
export default function RoofingDifferentiators() {
  return (
    <section id="why-nurturink" style={{ background: '#faf9f6', padding: '80px 40px' }}>
      <div style={{ maxWidth: '1060px', margin: '0 auto' }}>
        ...
        <div className="roofing-diff-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '22px' }}>
          {cards.map((c, i) => <RoofingDiffCard key={i} icon={c.icon} title={c.title} body={c.body} />)}
        </div>
      </div>
      <style>{`
        @media (max-width: 768px) {
          .roofing-diff-grid { grid-template-columns: 1fr !important; }
