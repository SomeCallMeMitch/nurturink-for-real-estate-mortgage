...
import RoofingHeroStatCard from './RoofingHeroStatCard';

/**
 * RoofingHero — SECTION 2: HERO
 * Cloned from SolarHero
 */
const stats = [
...
export default function RoofingHero() {
  return (
    <section style={{ background: '#213659', padding: '88px 40px 80px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="roofing-hero-inner" style={{ maxWidth: '1060px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 400px', gap: '72px', alignItems: 'center', position: 'relative', zIndex: 1 }}>
        {/* Left column */}
        <div>
          <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <RoofingBtnOrange href="#get-sample">Get Your Free Sample Card</RoofingBtnOrange>
            <RoofingBtnOutline href="#calculator">See the ROI Math</RoofingBtnOutline>
          </div>
        </div>

        {/* Right column — stats */}
        <div className="roofing-hero-stats" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
          {stats.map((s, i) => <RoofingHeroStatCard key={i} value={s.value} label={s.label} />)}
        </div>
      </div>

      <style>{`
        @media (max-width: 960px) {
          .roofing-hero-inner { grid-template-columns: 1fr !important; }
          .roofing-hero-stats { display: none !important; }
        }
...
function RoofingBtnOrange({ href, children }) {
  return (
...
function RoofingBtnOutline({ href, children }) {
  return (
    <a href={href} className="font-lato" style={{
      display: 'inline-block', padding: '13px 28px', borderRadius: '4px',
      fontSize: '15px', fontWeight: 700, letterSpacing: '0.02em',
