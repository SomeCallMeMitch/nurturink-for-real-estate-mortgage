...
- Whether the visible Save Draft controls in the bottom action bars are correctly wired.
...
- The Save Draft buttons now live in the existing bottom action areas, not the shared workflow header.
...
- Targeted eslint passed for `src/pages/FindClients.jsx`, `src/pages/SelectDesign.jsx`, `src/pages/ReviewAndSend.jsx`, and `src/components/mailing/WorkflowSteps.jsx`.
...
@@ -524,6 +690,72 @@ export default function FindClients() {
             </div>
           )}
...
@@ -958,6 +1190,24 @@ export default function FindClients() {
             </div>
...
@@ -1012,4 +1262,4 @@ export default function FindClients() {
       />
     </div>
   );
-}
