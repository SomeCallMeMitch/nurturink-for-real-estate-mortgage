...
const sampleTemplates = [
  {
    ...
    status: "approved",
    isDefault: false,
    categoryNames: ["Thank You", "Storm Response"]
  },
  {
  ...
  },
  {
    ...
    status: "approved",
    isDefault: false,
    categoryNames: ["Referral Request", "Follow-Up"]
  },
  {
  ...
  },
  {
    ...
    status: "approved",
    isDefault: false,
    categoryNames: ["Lead Nurture", "Follow-Up"]
  },
  {
    ...
    status: "approved",
    isDefault: false,
    categoryNames: ["Storm Response", "Lead Nurture"]
  },
  {
    name: "Test Template",
...
Deno.serve(async (req) => {
  try {
    ...
    // Check if any templates already exist for this organization
    const existingTemplates = await base44.entities.Template.filter({
      orgId: user.orgId
    });
    ...
    if (existingTemplates.length > 0) {
      return Response.json({
        success: false,
        message: `Templates already exist. Found ${existingTemplates.length} existing templates.`,
        templateCount: existingTemplates.length
      });
    }
    
    // Load all template categories to map names to IDs
    const allCategories = await base44.entities.TemplateCategory.list();
    const categoryMap = {};
    ...
    for (const sampleTemplate of sampleTemplates) {
      const { categoryNames, ...templateData } = sampleTemplate;
      
      // Convert category names to IDs
      const templateCategoryIds = categoryNames
