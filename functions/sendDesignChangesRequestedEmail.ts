...
<head>
  ...
  <title>Design Changes Requested</title>
</head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; max-width: 100%; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); padding: 40px 40px 30px; text-align: center;">
              ...
              <span style="display: inline-block; background-color: rgba(255,255,255,0.2); color: #ffffff; padding: 6px 16px; border-radius: 4px; font-size: 12px; font-weight: bold;">CHANGES REQUESTED</span>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <h2 style="margin: 0 0 20px; font-size: 20px; font-weight: bold; color: #1f2937;">
                ${requester_fullName} requested changes to the custom design
              </h2>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #e5e7eb; border-radius: 8px; margin-bottom: 30px;">
                ...
                <tr style="background-color: #f9fafb;">
                  <td style="padding: 12px 20px; font-size: 14px; color: #6b7280;">Requested By</td>
                  <td style="padding: 12px 20px; font-size: 14px; color: #111827; text-align: right;">${requester_fullName}</td>
                </tr>
              </table>
              
              <!-- Comments Box -->
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #fef3c7; border: 2px solid #fbbf24; border-radius: 8px; margin-bottom: 30px;">
                <tr>
                  <td style="padding: 20px;">
                    <p style="margin: 0 0 10px; font-size: 14px; font-weight: bold; color: #92400e;">Requested Changes:</p>
                    <p style="margin: 0; font-size: 14px; color: #374151; line-height: 1.6;">${change_comments}</p>
                  </td>
                </tr>
              </table>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="text-align: center;">
                    <a href="${design_request_url}" style="display: inline-block; background-color: #8b5cf6; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-size: 16px; font-weight: bold; box-shadow: 0 4px 6px rgba(139, 92, 246, 0.3);">View & Update Design</a>
                  </td>
                </tr>
              </table>
            </td>
...
const createDesignChangesText = (props) => `
RoofScribe - Design Changes Requested

Hi ${props.admin_firstName},

${props.requester_fullName} requested changes to the custom design

DETAILS:
Design Name: ${props.design_name}
Request ID: ${props.design_request_id}
Requested By: ${props.requester_fullName}

REQUESTED CHANGES:
${props.change_comments}

View & Update Design: ${props.design_request_url}
...
Deno.serve(async (req) => {
  try {
    ...
    const emailData = {
      ...
      change_comments: data.change_comments,
      requested_date: data.requested_date || new Date().toLocaleDateString('en-US'),
    ...
    const result = await resend.emails.send({
      ...
      to: data.admin_email,
      subject: `${data.requester_fullName} requested changes to the custom design`,
      html: createDesignChangesHTML(emailData),
      text: createDesignChangesText(emailData)
    });

    return Response.json({ 
      success: true, 
      emailId: result.data?.id,
      message: 'Design changes notification sent' 
    });

  } catch (error) {
    console.error('Error sending design changes notification:', error);
