...
/**
 * Invite a new team member to the organization - V2 STANDALONE
 * This version inlines all role helpers to ensure fresh deployment
 * 
 * @version 2026-01-25-v2-standalone
 */
...
Deno.serve(async (req) => {
  console.log('=== inviteTeamMemberV2 STANDALONE START ===');
  console.log('BUILD_ID: 2026-01-25-v2-standalone');
  ...
  try {
    ...
    console.log('=== REQUEST BODY DEBUG V2-STANDALONE ===');
    ...
    if (orgRole) {
      ...
      if (!validOrgRoles.includes(orgRole)) {
        console.log('VALIDATION FAILED V2: Invalid orgRole. Valid roles:', validOrgRoles, 'Received:', orgRole);
        return Response.json(
          { error: `Invalid orgRole "${orgRole}". Must be "owner", "manager", or "member" (V2-standalone)` },
          { status: 400 }
        );
      }
    } 
    // Handle legacy role field (backwards compatibility)
      ...
      if (!['sales_rep', 'organization_owner', 'organization_manager'].includes(role)) {
        console.log('VALIDATION FAILED V2: Invalid legacy role');
        return Response.json(
          { error: 'Invalid role. Must be "sales_rep", "organization_manager", or "organization_owner" (V2-standalone)' },
          { status: 400 }
        );
      }
    ...
    if (existingUsers.length > 0) {
      ...
      console.log('=== inviteTeamMemberV2 SUCCESS (existing user) ===');
      return Response.json({
        success: true,
    ...
    console.log('=== inviteTeamMemberV2 SUCCESS (new invitation) ===');
    return Response.json({
      success: true,
    ...
    console.error('=== inviteTeamMemberV2 ERROR ===');
    console.error('Error in inviteTeamMemberV2:', error);
